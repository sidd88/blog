...
**Step 2**: Create `config` file in `~/.ssh` directory (if not exist) and add host for multiple account to config host entries:
...
And finally, config `user.name`, `user.email` for local repository corresponding with github account.
